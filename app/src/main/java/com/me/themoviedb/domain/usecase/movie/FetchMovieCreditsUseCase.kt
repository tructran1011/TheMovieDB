...
) : FlowUseCase<Int, MovieCredits>(coroutineDispatcher) {

    override fun execute(parameter: Int): Flow<Result<MovieCredits>> {
        return flow {
            emit(Result.loading())
            emit(movieRepository.getMovieCredits(parameter))
        }
    }
}